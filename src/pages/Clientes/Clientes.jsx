...
export default function Clientes() {
  ...
  const statsPorCliente = useMemo(() => {
    ...
    for (const m of movimientos) {
      ...
      if (!mapa.has(m.cliente_id)) mapa.set(m.cliente_id, {})
      const porMoneda = mapa.get(m.cliente_id)
  ...
  return (
    <DashboardLayout>
      <div className="mb-6">
        <h1 className="font-display text-2xl font-semibold text-white">Clientes</h1>
        <p className="text-metal-300">
          Contactos y facturación asociada a tus ingresos ({clientes.length} cargados).
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          {loading ? (
            ...
            <p className="card text-sm text-metal-400">Todavía no cargaste clientes.</p>
          ) : (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {clientes.map((cliente) => {
                const stats = statsPorCliente.get(cliente.id)
                return (
                  <div key={cliente.id} className="card">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="truncate font-medium text-white">{cliente.nombre}</p>
                        {cliente.email && (
                          <p className="truncate text-xs text-metal-400">{cliente.email}</p>
                        )}
                        {cliente.telefono && (
                          <p className="text-xs text-metal-400">{cliente.telefono}</p>
                        )}
                      </div>
                      ...
                      <div className="mt-3 space-y-1 border-t border-metal-800 pt-3">
                        {Object.entries(stats).map(([moneda, s]) => (
                          <div key={moneda} className="flex items-center justify-between text-sm">
                            <span className="text-metal-400">Pendiente de cobro</span>
                            <span
                              className={
